...
const PinnedNotesPage = ({ refreshAll }) => {
  const {
    notes, loading,
  ...
  return (
    <>
      <div className="db-topbar">
        <span className="db-topbar-title">Pinned</span>
        <div className="db-search-wrap">
          <span className="db-search-icon"><FiSearch size={13} /></span>
          <input className="db-search" placeholder="Search pinned notes…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
        ...
        <div className="db-notes-pane">
          {loading ? (
            ...
            <div className="db-empty">
              <div className="db-empty-icon"><FiBookmark size={36} /></div>
              <h3>No pinned notes</h3>
              <p>Pin a note to keep it close at hand.</p>
            </div>
          ) : (
