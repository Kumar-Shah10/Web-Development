...
const FavoriteNotesPage = ({ refreshAll }) => {
  const {
    notes, loading,
  ...
  const handleToggleFavorite = async (id) => {
    try {
      ...
      if (activeNote?.id === id) setActiveNote(null); // remove from view if unfavorited
      fetchCurrentNotes();
      refreshAll();
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <>
      <div className="db-topbar">
        <span className="db-topbar-title">Favorites</span>
        ...
        <div className="db-notes-pane">
          {loading ? (
            ...
            <div className="db-empty">
              <div className="db-empty-icon"><FiHeart size={36} /></div>
              <h3>No favorites yet</h3>
              <p>Save notes you love to find them quickly here.</p>
            </div>
          ) : (
